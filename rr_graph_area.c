...
static double get_cblock_trans(int* num_inputs_to_cblock, int
                              max_inputs_to_cblock, double trans_cblock_to_lblock_buf,
                              double trans_sram_bit);

static double* alloc_and_load_unsharable_switch_trans(int num_switch,
                                                     double trans_sram_bit, double R_minW_nmos);

static double* alloc_and_load_sharable_switch_trans(int num_switch,
                                                   double trans_sram_bit, double R_minW_nmos, double R_minW_pmos);

static double trans_per_buf(double Rbuf, double R_minW_nmos, double R_minW_pmos);

static double trans_per_mux(int num_inputs, double trans_sram_bit);

static double trans_per_R(double Rtrans, double R_minW_trans);


/*************************** Subroutine definitions **************************/


void count_routing_transistors(int num_switch, double R_minW_nmos,
                               double R_minW_pmos)
{
                              ...
                              double trans_sram_bit)
{
    ...
    double trans_count;
    int i, num_inputs;
    trans_per_cblock = (double*) my_malloc((max_inputs_to_cblock + 1) *
                                          sizeof(double));
                                                     ...
                                                     double trans_sram_bit, double R_minW_nmos)
{
    ...
    double* unsharable_switch_trans, Rpass;
    int i;
    unsharable_switch_trans = (double*) my_malloc(num_switch * sizeof(double));
                                                   ...
                                                   double trans_sram_bit, double R_minW_nmos, double R_minW_pmos)
{
    ...
    double* sharable_switch_trans, Rbuf;
    int i;
    sharable_switch_trans = (double*) my_malloc(num_switch * sizeof(double));
...
static double trans_per_buf(double Rbuf, double R_minW_nmos, double R_minW_pmos)
{
    ...
    double trans_count, stage_ratio, Rstage;
    ...
    if (Rbuf > 0.6 * R_minW_nmos || Rbuf <= 0.) { /* Use a single-stage buffer */
        ...
        stage_ratio = pow(R_minW_nmos / Rbuf, 1. / (double) num_stage);
        Rstage = R_minW_nmos;
        trans_count = 0.;
