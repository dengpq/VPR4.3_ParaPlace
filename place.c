...
static int try_swap(const placer_paras_t*  placer_paras_ptr,
                    ...
                    placer_costs_t*  placer_costs_ptr)
{
    /* 1) Set from_block(x_from, y_from, z_from) and to_block(x_to, y_to, z_to) * 
    ...
    for (k = 0; k < num_nets_affected; ++k) {
        ...
        if (net[inet].num_net_pins <= SMALL_NET) { /* 4 */
                       ...
                       int num_regions)
{
    ...
    for (n = 0; n < num_nets; ++n) { /* for each net ... */
        if (is_global[n] == FALSE) { /* Do only if not global. */
            ...
            if (net[n].num_net_pins > SMALL_NET && method == NORMAL) { /* 4 */
                        ...
                        placer_costs_t* placer_costs_ptr)
{
    ...
    int* block_done = (int*)my_malloc(num_blocks * sizeof(int));
...
}  /* end of void check_place(double bb_cost, ) */
