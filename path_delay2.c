...
vertex_t* tnode;                 /* [0..num_tnodes - 1] */
vertex_descript* tnode_descript; /* [0..num_tnodes - 1] */
...
static int* alloc_and_load_tnode_fanin_and_check_edges(int* num_sinks_ptr)
{
    ...
    int* tnode_num_fanin = (int*)my_calloc(num_tnodes, sizeof(int));
    int error = 0;
    int num_sinks = 0;
...
/* FIXME: allocate and load timing_graph edges and levels */
int alloc_and_load_timing_graph_levels(void)
{
    ...
    return num_sinks;
} /* end of int alloc_and_load_timing_graph_levels() */
...
double print_critical_path_node(FILE* fp, t_linked_int* critical_path_node)
{
     ...
     * the next node.                                                           */
    int inet, downstream_node;
    static char* tnode_type_names[] = {"INPAD_SOURCE",
                                       "INPAD_OPIN",
                                       "OUTPAD_IPIN",
                                       "OUTPAD_SINK",
                                       "CLB_IPIN",
    ...
    double Tdel;
    int inode = critical_path_node->data;
    int iblk = tnode_descript[inode].iblk;
    int ipin = tnode_descript[inode].ipin;
    vertex_type_t type = tnode_descript[inode].type;
