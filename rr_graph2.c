                                          ...
                                          int max_len)
{
    ...
    for (i = 0; i < num_seg_types; i++) {
        ...
        if (!longline) {
            ...
            start_step = (double) length / (double) ntracks;
        } else {                               /* Is a long line. */
            length = max_len;
            start_step = 0.;
        }
        ...
        if (!longline) {
            if (num_cb > 1) {
                cb_step = (double)(length - 1) / (double)(num_cb - 1);
            } else {
                cb_step = 0.;
            }
        }
         ...
         * would cause stupidities otherwise.                                       */
            ...
            if (num_cb > 0) {
                cb_step = (double) length / (double) num_cb;
            } else {
                cb_step = 0.;
            }
        }
        ...
        if (!longline) {
            if (num_sb > 1) {
                sb_step = (double) length / (double)(num_sb - 1);
            } else {
                sb_step = 0.;
            }
        } else {  /* Is a longline */
            if (num_sb > 0) {
                sb_step = (double)(length + 1) / (double) num_sb;
            } else {
                sb_step = 0;
            }
        }
        ...
        if (!longline) {
            ...
            cb_off_step = cb_step / (double) ntracks;
            sb_off_step = sb_step / (double) ntracks;
        }
                      ...
                      int nodes_per_chan, int** rr_node_indices)
{
    ...
    int iclass = 0;
    switch (bin_grids[i][j].grid_type) {
        case B_CLB_TYPE:
            switch (rr_type) {
                case SOURCE:
