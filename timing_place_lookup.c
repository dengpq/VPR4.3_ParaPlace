...
static double assign_blocks_and_route_net(enum e_block_types source_type,
                                         int source_x_loc, int source_y_loc,
                                         enum e_block_types sink_type,
                                         int sink_x_loc, int sink_y_loc,
                                                 ...
                                                 int* original_num_blocks)
{
    ...
    *original_num_blocks = num_blocks;
    ...
    net_delay = (double**) alloc_matrix(0, NET_COUNT - 1, 1, BLOCK_COUNT - 1, sizeof(double));
    net_slack = (double**) alloc_matrix(0, NET_COUNT - 1, 1, BLOCK_COUNT - 1, sizeof(double));
    init_occ();
}
...
{
    int id_x, id_y;
    delta_clb_to_clb = (double**) alloc_matrix(0, nx - 1, 0, ny - 1, sizeof(double));
    delta_inpad_to_clb = (double**) alloc_matrix(0, nx, 0, ny, sizeof(double));
    delta_clb_to_outpad = (double**) alloc_matrix(0, nx, 0, ny, sizeof(double));
    delta_inpad_to_outpad = (double**) alloc_matrix(0, nx + 1, 0, ny + 1, sizeof(double));
...
static void free_delta_arrays(void)
{
    free_matrix(delta_inpad_to_clb, 0, nx, 0, sizeof(double));
    free_matrix(delta_clb_to_clb, 0, nx - 1, 0, sizeof(double));
    free_matrix(delta_clb_to_outpad, 0, nx, 0, sizeof(double));
    free_matrix(delta_inpad_to_outpad, 0, nx + 1, 0, sizeof(double));
}
/**************************************/
static void generic_compute_matrix(double** *matrix_ptr,
                                   enum e_block_types source_type,
...
static void compute_delta_inpad_to_outpad(struct s_router_opts router_opts,
                                          ...
                                          t_timing_inf timing_inf)
{
    enum e_block_types source_type = INPAD;
    enum e_block_types sink_type = OUTPAD;
...
/*************************************************************
 * Main function in timing_place_lookup.c                    *
 * Compute 4 delay_lookup_matrix for TIMING_DRIVEN_PLACEMENT *
 ************************************************************/
                                 ...
                                 t_subblock_data subblock_data)
{
     ...
     * are using the net structure in these routines to find delays *
     * between blocks.                                              */
    static struct s_net* original_net;
    /*original_block: same def as original_nets, but for block  */
    static struct s_block* original_block;
    ...
    alloc_routing_structs(router_opts, det_routing_arch, segment_inf, timing_inf,
                          subblock_data);
