...
static boolean breadth_first_route_net(int inet, double bend_cost);
                                ...
                                vector_t** clb_opins_used_locally)
{
    ...
    double pres_fac = router_opts.first_iter_pres_fac;

    int itry, inet;
    for (itry = 1; itry <= router_opts.max_router_iterations; ++itry) {
        /* try to route net one by one according to same sequence */
        for (inet = 0; inet < num_nets; ++inet) {
            if (is_global[inet] == FALSE) { /* Skip global nets. */
                pathfinder_update_one_cost(trace_head[inet], -1, pres_fac);

                boolean is_routable = breadth_first_route_net(inet,
    ...
    } /* end of for (max_router_iterations)*/

    printf("Routing failed.\n");
    return (FALSE);
} /* end of boolean try_breadth_first_route(Wirelength-Driven Router) */
...
static boolean breadth_first_route_net(int inet, double bend_cost)
{
    ...
    mark_ends(inet);
    struct s_trace* trace_node = NULL;
    ...
    reset_path_costs();
    return (TRUE);
} /* end of static boolean breadth_first_route_net(route a net) */
                                               ...
                                               int remaining_connections_to_sink)
{
    ...
    if (remaining_connections_to_sink == 0) {   /* Usual case. */
        while (tptr != NULL) {
            /* create heaps for every pin node */
        ...
        struct s_trace* next_ptr = tptr->next;
        int last_ipin_node = OPEN; /* Stops compiler from complaining. */
    ...
    } /* end of else(remaining_connections_to_sink != 0) */
