 ...
 *       or subblock pin is a timing node(tnode). The connectivity between *
 *       pins is represented by timing edges(tedges). All delay is marked  *
                                     ...
                                     t_subblock_data subblock_data)
{
     ...
     *       It initial all the pin-net connections in circuit netlist block   *
     *       or subblocks.                                                     */
    ...
    int*** sblk_pin_to_tnode; /* OUTPUT, INPUT, CLOCK */

    /* Make mapping from all blocks' pins and its subblocks pins to tnode. *
     * When using C++ write the program, I will use map<int, int>.         */
    ...
    /* FIXME: Then initial all tnodes and tedges to create Timing_Graph */
    ...
    /* Initial the Timing_Graph levels */
    int num_sinks = alloc_and_load_timing_graph_levels();
...
/* Allocates and loads the block and subblk pin mapping to tnode    *
                                       ...
                                       int** num_uses_of_sblk_opin)
{
    ...
    t_subblock** subblock_inf = subblock_data.subblock_inf;
    ...
    for (iblk = 0; iblk < num_blocks; ++iblk) {
        if (block[iblk].type == CLB) {
            ...
            /* Now do all subblocks in current netlist's block mapping. */
            ...
            for (isub = 0; isub < num_subblocks; ++isub) {
                ...
                if (subblock_inf[iblk][isub].clock != OPEN) {
                   /* If this is a sequential block, we have two more pins: *
                                         ...
                                         t_subblock_data subblock_data)
{
    ...
    /* number of subblocks in each block. for io pads, it was 0. For a CLB, it *
     * was from 1 to max_subblocks_per_block.                                  *
     * int num_subblocks_per_block[0...num_blocks-1].                         */
    int* num_subblocks_per_block = subblock_data.num_subblocks_per_block;

    /* t_subblock subblock_inf[0...num_blocks-1][0...num_subblocks[iblk]-1] */
    t_subblock** subblock_inf = subblock_data.subblock_inf;
                                                  ...
                                                  t_timing_inf timing_inf)
{
    ...
    tnode = (vertex_t*)my_malloc(num_tnodes * sizeof(vertex_t));
    tnode_descript = (vertex_descript*)my_malloc(num_tnodes *
                                                 sizeof(vertex_descript));
    /* Attention, tedge in Timing-Analyze-Graph was depend on point-to-point *
     * connection, not net.                                                  */
    net_to_driver_tnode = (int*)my_malloc(num_nets * sizeof(int)); /* <net, tnode> */
    ...
    int iblk = -1;
    for (iblk = 0; iblk < num_blocks; ++iblk) {
        switch (block[iblk].type) { /* netlist block type: CLB, INPAD, OUTPAD */
            case CLB:
                             ...
                             int*   next_clb_ipin_edge)
{
    ...
    for (ipin = 0; ipin < pins_per_clb; ++ipin) {
        ...
        if (inode != OPEN) { /* Pin is used -> put in graph */
            if (is_opin(ipin)) { /* block output pin */
                build_block_output_tnode(inode,
                                         iblk,
                                         ipin,
                                         block_pin_to_tnode);
                ...
                tnode[inode].out_edges = (edge_t*)my_chunk_malloc(num_edges * sizeof(edge_t),
                                                                  &tedge_ch_list_head,
                                                                  &tedge_ch_bytes_avail,
                                                                  &tedge_ch_next_avail);
        ...
        } /* end of if (inode != OPEN) */
    } /* end of for(ipin = 0; ipin < pins_per_clb; ++ipin) */

    /* Then load the edge arrays for the CLB input pins. Do this by looking at *
    ...
    for (isub = 0; isub < num_subs; ++isub) {
        ...
        if (from_pin != OPEN && from_pin < pins_per_clb) {
            ...
            iedge = next_clb_ipin_edge[from_pin]++; /* ? */
            tedge[iedge].to_node = to_node;
            /* For the earliest possible clock I want this delay to be zero,     *
             * so it arrives at flip flops at T = 0. For later clocks or locally *
             * generated clocks that may accumulate delay(like the clocks in a   *
             * ripple counter), I might want to make this delay nonzero. Not worth*
             * bothering about now.  */
            tedge[iedge].Tdel = 0.0;
        }
    } /* end of for(isub = 0; isub < num_subs; isub++) */
...
static void build_subblock_tnodes(int* n_uses_of_sblk_opin,
                                  int* blk_pin_to_tnode,
                                  int** sub_pin_to_tnode, /* subblock pin index to tnode */
                                  int  subblock_lut_size,
                                  int  num_subs,
                                  t_subblock* sub_inf,
                                  double T_sblk_opin_to_sblk_ipin,
                                  ...
                                  int iblk)
{
    ...
    int ipin = -1;
    for (isub = 0; isub < num_subs; ++isub) {
        inode = sub_pin_to_tnode[isub][out_pin]; /* output node */
    ...
    for (isub = 0; isub < num_subs; isub++) {
        ...
        if (sub_inf[isub].clock == OPEN) { /* Combinational mode */
            ...
            tnode[inode].out_edges = (edge_t*)my_chunk_malloc(sizeof(edge_t),
                                                              &tedge_ch_list_head,
                                                              &tedge_ch_bytes_avail,
                                                              &tedge_ch_next_avail);
            tedge = tnode[inode].out_edges;
                              ...
                              t_subblock** subblock_inf)
{
    ...
    int inode = block_pin_to_tnode[iblk][0];
...
} /* end of static void build_ipad_tnode() */
...
/* Initialize all tedges's delay in Timing_Analyze_Graph using double** net_delay. */
/* Sets the delays of the inter-CLB nets to the values specified by (double**)     *
 * net_delay[0..num_nets-1][1..num_pins-1]. These net delays should have been      *
 * allocated and loaded with the net_delay routines. This routine marks the corres-*
 * ponding edges in the timing graph with the proper delay.                        */
 ...
 * delay for the circuit -- if 0, the target_cycle_time is set to the crit. path  *
 * found in the timing graph. This routine loads net_slack, and returns the current *
 * crit. path delay. If a source-sink pair in timing-graph slack was 0, it was a  *
                      ...
                      double target_cycle_time)
{
    ...
    int to_node = -1;
    edge_t* tedge = NULL;
    /* use breath-first-search to calculate the arr_time, T_c*/
    for (ilevel = 0; ilevel < num_tnode_levels; ++ilevel) {
        ...
        for (i = 0; i < num_at_level; ++i) {
            ...
            for (iedge = 0; iedge < num_edges; ++iedge) {
                ...
                tnode[to_node].arr_time = max(tnode[to_node].arr_time,
                                              arr_time + Tdel);
            }
        }
    ...
    if (target_cycle_time > 0.0) {
        ...
        /* In this circumstance, the default fmax was the max-delay. */
        T_cycle = T_crit;
    }
    ...
    /* Then calculate all tedge's slack in Timing_Analyze_Graph */
    compute_net_slacks(net_slack);
    return T_crit;
} /* end of double load_net_slack(double** net_slack, double target_cycle_time) */
...
static void compute_net_slacks(double** net_slack)
{
    ...
    for (inet = 0; inet < num_nets; ++inet) {
        int inode = net_to_driver_tnode[inet];
        double arr_time = tnode[inode].arr_time;
        ...
        for (iedge = 0; iedge < num_edges; ++iedge) {
            ...
            double req_time = tnode[to_node].req_time;
                                           ...
                                           double constant_net_delay_value)
{
    ...
    double** net_slack = alloc_and_load_timing_graph(timing_inf,
                                                     subblock_data);
