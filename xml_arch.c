...
static char** GetNodeTokens(IN ezxml_t Node)
{
    ...
    char** Tokens = (char**)my_malloc(sizeof(char*) * (Count + 1));
    char* Dst = (char*)my_malloc(sizeof(char) * Len);
    Count = 0;
    /* Copy data to tokens */
    char* Cur = Node->txt;
    boolean InToken = FALSE;
                           ...
                           IN boolean Required)
{
    ...
    ezxml_t Cur = ezxml_child(Parent, Name);
...
static const char* FindProperty(IN ezxml_t Parent,
                                ...
                                IN boolean Required)
{
    const char* Res = ezxml_attr(Parent, Name);
    if (Required) {
        if (NULL == Res) {
            printf(ERRTAG
...
static void ParseFc(ezxml_t Node,
                    ...
                    double* Val)
{
    const char* Prop = FindProperty(Node, "type", TRUE);
                          ...
                          t_type_descriptor* Type)
{
    ...
    Type->class_inf = (t_class*)my_malloc(Type->num_class * sizeof(t_class));
    ...
    for (i = 0; i < Type->capacity; ++i) {
        ...
        while (Cur != NULL) {
            ...
            const char* Prop = FindProperty(Cur,
                                            "type",
                                            TRUE);
            if (0 == strcmp(Prop, "in")) {
                            ...
                            t_type_descriptor* Type)
{
    ...
    while (Cur != NULL) {
        ...
        const char* Prop = FindProperty(Cur, "type", TRUE);
        ...
        char** Tokens = GetNodeTokens(Cur);
        assert(CountTokens(Tokens) == Type->class_inf[CurClass].num_pins);

        for (j = 0; j < Type->class_inf[CurClass].num_pins; ++j) {
            int CurPin = my_atoi(Tokens[j]);
            ...
            for (i = 0; i < Type->capacity; ++i) {
                ...
                Type->is_global_pin[CurPin + (i * PinsPerSubtile)] = IsGlobal;
            }
        }
...
}  /* end of static void SetupPinClasses(ezxml_t Classes, ) */
                              ...
                              t_type_descriptor* Type)
{
    ...
    ezxml_t Cur = Locations->child;
    while (Cur != NULL) {
        CheckElement(Cur, "loc");
        /* Get offset */
        i = 0;
        const char* Prop = FindProperty(Cur, "offset", FALSE);
        if (Prop != NULL) {
            i = my_atoi(Prop);
                               ...
                               t_type_descriptor* Type)
{
    ...
    Type->grid_loc_def = (struct s_grid_loc_def*)my_calloc(Type->num_grid_loc_def,
                            ...
                            t_type_descriptor* Type)
{
    ...
    while (Cur) {
        ...
        if (0 == strcmp(Prop, "T_fb_ipin_to_sblk_ipin")) {
            ...
            printf(ERRTAG "'%s' is an unrecognized timing tedge.\n",
                   Prop);
            exit(1);
        }
...
static void SetupSubblocksTcomb(ezxml_t timing,
                                t_type_descriptor* Type)
{
    t_T_subblock** T_subblock_ptr = &Type->type_timing_inf.T_subblock;

    double** t_comb = (double**)alloc_matrix(0, Type->max_subblock_inputs - 1,
                                             0, Type->max_subblock_outputs - 1,
                                             sizeof(double));
    int i, j;
...
}  /* end of static void SetupSubblocksTcomb(ezxml_t timing,) */
                               ...
                               t_type_descriptor* Type)
{
    ...
    double* t_seq[2];
    t_seq[0] = (double*)my_calloc(Type->max_subblock_inputs, sizeof(double));
    t_seq[1] = (double*)my_calloc(Type->max_subblock_outputs, sizeof(double));

    /* Load the timing info */
    int i = -1;
    for (i = 0; i < 2; ++i) {
        ezxml_t Cur = seq_timing[i]->child;
        int j = 0;
...
}  /* end of static void SetupSubblocksTSeq(ezxml_t timing_seq_in, ) */
...
} /* end of static void Process_Fc(ezxml_t Fc_in_node, ) */
                             ...
                             boolean timing_enabled)
{
    ...
    const char* Prop = FindProperty(Parent, "max_subblocks", FALSE);
    if (Prop) {
    ...
    ezxml_t CurType = FindElement(Parent, "timing", timing_enabled);
    if (CurType != NULL) {
        Type->type_timing_inf.T_subblock = (t_T_subblock*)my_malloc(Type->max_subblocks *
                                                                    sizeof(t_T_subblock));
        /* Load T_comb timing for subblock */
        ezxml_t Cur = FindElement(CurType, "T_comb", TRUE);
        SetupSubblocksTcomb(Cur, Type);
        FreeNode(Cur);
        /* Load Fc */
        Cur = FindElement(CurType, "T_seq_in", TRUE);
        ezxml_t Cur2 = FindElement(CurType, "T_seq_out", TRUE);
...
{
    /* Load type name */
    const char* Prop = FindProperty(Node, "name", TRUE);
...
static void ProcessDevice(INOUT ezxml_t Node,
                          ...
                          IN boolean timing_enabled)
{
    ezxml_t Cur = FindElement(Node, "sizing", TRUE);
    arch->R_minW_nmos = 0;
    const char* Prop = FindProperty(Cur, "R_minW_nmos", timing_enabled);
    if (Prop != NULL) {
...
static void ProcessChanWidthDistr(INOUT ezxml_t Node,
                                  OUT struct s_arch* arch)
{
    ezxml_t Cur = FindElement(Node, "io", TRUE);
    const char* Prop = FindProperty(Cur, "width", TRUE);
    ...
    FreeNode(Cur);
}  /* end of static void ProcessChanWidthDistr(INOUT ezxml_t Node,) */
                                     ...
                                     OUT t_chan* chan)
{
    ...
    const char* Prop = FindProperty(Node, "distr", TRUE);
...
static void ProcessIO(INOUT ezxml_t Node,
                      IN boolean timing_enabled)
{
    enum { INCLASS = 0, OUTCLASS = 1, CLKCLASS = 2 };
    CheckElement(Node, "io");
    t_type_descriptor* type = &type_descriptors[IO_TYPE->index];
    type->name = ".io";
    type->height = 1;
    /* Load capacity */
    const char* Prop = FindProperty(Node, "capacity", TRUE);
    int capacity = my_atoi(Prop);
    type->capacity = capacity;
    ezxml_set_attr(Node, "capacity", NULL);
    /* Load Fc */
    ezxml_t Cur = FindElement(Node, "fc_in", TRUE);
    ezxml_t Cur2 = FindElement(Node, "fc_out", TRUE);
    ...
    type->class_inf = (struct s_class*)my_malloc(sizeof(struct s_class) *
                                                   3 * capacity);
    ...
    for (j = 0; j < capacity; ++j) {
        ...
        type->class_inf[3 * j + INCLASS].pinlist = (int*)my_malloc(sizeof(int) * num_inputs);

        for (i = 0; i < num_inputs; ++i) {
            type->class_inf[3 * j + INCLASS].pinlist[i] = num_pins * j / capacity + i;
            type->pin_class[num_pins * j / capacity + i] = 3 * j + INCLASS;
    ...
    type->grid_loc_def = (struct s_grid_loc_def*)my_calloc(1, sizeof(struct s_grid_loc_def));
    ...
    if (timing_enabled) {
        ...
        type->type_timing_inf.T_sblk_opin_to_fb_opin  =  0;
        type->type_timing_inf.T_sblk_opin_to_sblk_ipin  = 0;
        type->type_timing_inf.T_subblock  =  (t_T_subblock*)my_malloc(sizeof(t_T_subblock));
        type->type_timing_inf.T_subblock[0].T_comb  =  (double**)my_malloc(sizeof(double*));
        type->type_timing_inf.T_subblock[0].T_comb[0]  =  (double*)my_malloc(sizeof(double));
        type->type_timing_inf.T_subblock[0].T_comb[0][0]  =  t_out;
        type->type_timing_inf.T_subblock[0].T_seq_in  =  (double*)my_malloc(sizeof(double));
        type->type_timing_inf.T_subblock[0].T_seq_in[0]  =  0;
        type->type_timing_inf.T_subblock[0].T_seq_out  =  (double*)my_malloc(sizeof(double));
                         ...
                         boolean timing_enabled)
{
    ...
    *Types = (t_type_descriptor*)my_malloc(sizeof(t_type_descriptor) * (*NumTypes));
    ...
    FreeNode(Cur);
    /* Process the types */
    int i = 2;  /* Skip over 'empty' and 'io' type */

    ezxml_t CurType = Node->child;
    while (CurType != NULL) {
        ...
        ezxml_t Cur2 = FindElement(CurType, "fc_out", TRUE);
        Process_Fc(Cur, Cur2, Type);
        FreeNode(Cur);
        FreeNode(Cur2);
...
{
    /* Parse the file */
    ezxml_t Cur = ezxml_parse_file(ArchFile);
    if (NULL == Cur) {
    ...
    /* Process layout */
    ezxml_t Next = FindElement(Cur, "layout", TRUE);
                            ...
                            IN boolean timing_enabled)
{
    ...
    *NumSegs = CountChildren(Parent, "segment");
    if (*NumSegs > 0) {
        *Segs = (struct s_segment_inf*)my_malloc(*NumSegs * sizeof(struct s_segment_inf));
    ...
    ezxml_t SubElem;
    for (i = 0; i < *NumSegs; ++i) {
        ezxml_t Node = ezxml_child(Parent, "segment");
        /* Get segment length */
        int length = 1; /* DEFAULT */
        const char* tmp = FindProperty(Node, "length", FALSE);
        if (tmp) {
                         ...
                         IN int len)
{
    ...
    const char* tmp = FindProperty(Node, "type", TRUE);
...
static void ProcessSwitches(INOUT ezxml_t Parent,
                            OUT struct s_switch_inf** Switches,
                            OUT int* NumSwitches,
                            IN boolean timing_enabled)
{
    ...
    *Switches = NULL;
    ...
    if (*NumSwitches > 0) {
        *Switches = (struct s_switch_inf*)my_malloc(*NumSwitches * sizeof(struct s_switch_inf));
        memset(*Switches, 0, (*NumSwitches * sizeof(struct s_switch_inf)));
    }

    /* Load the switches. */
    boolean has_buf_size = FALSE;
    int i, j;
    for (i = 0; i < *NumSwitches; ++i) {
        ezxml_t Node = ezxml_child(Parent, "switch");
        const char* switch_name = FindProperty(Node, "name", TRUE);
        const char* type_name = FindProperty(Node, "type", TRUE);
        ...
        const char* Prop = FindProperty(Node, "R", timing_enabled);
        if (Prop != NULL) {
...
} /* end of static void ProcessSwitches(INOUT ezxml_t Parent,) */
