...
#define XTOWORLD(x) (((double) x)/xmult + xleft)
#define YTOWORLD(y) (((double) y)/ymult + ytop)
...
static double xleft, xright, ytop, ybot;         /* world coordinates */
/* Initial world coordinates */
static double saved_xleft, saved_xright, saved_ytop, saved_ybot;

static double ps_left, ps_right, ps_top, ps_bot; /* Figure boundaries for *
                        * PostScript output, in PostScript coordinates.  */
static double ps_xmult, ps_ymult;     /* Transformation for PostScript. */
static double xmult, ymult;                  /* Transformation factors */
...
static int xcoord(double worldx)
{
     ...
     * this problem is to do all the clipping in advance in doubleing point,  *
...
static int ycoord(double worldy)
{
    /* Translates from my internal coordinates to X Windows coordinates   *
...
static void update_transform(void)
{
    ...
    xmult = ((double) top_width - 1. - MWIDTH) / (xright - xleft);
    ymult = ((double) top_height - 1. - T_AREA_HEIGHT) / (ybot - ytop);
...
void event_loop(void (*act_on_button)(double x, double y),
...
static int rect_off_screen(double x1, double y1, double x2, double y2)
{
...
static double angnorm(double ang)
{
...
void drawarc(double xc, double yc, double rad, double startang,
             double angextent)
{
...
void fillarc(double xc, double yc, double rad, double startang,
             double angextent)
{
...
void drawtext(double xc, double yc, char* text, double boundx)
{
...
void update_message(char* msg) { }
void draw_message(void) { }
void init_world(double xl, double yt, double xr, double yb) { }
void flushinput(void) { }
void setcolor(int cindex) { }
void setlinestyle(int linestyle) { }
void setlinewidth(int linewidth) { }
...
void fillrect(double x1, double y1, double x2, double y2) { }
void fillpoly(t_point* points, int npoints) { }
void drawarc(double xcen, double ycen, double rad, double startang,
             double angextent) { }

void fillarc(double xcen, double ycen, double rad, double startang,
             double angextent) { }
