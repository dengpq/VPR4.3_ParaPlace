    ...
    switch (from_type) {
        case SOURCE:
            if (to_type == OPIN) {
                if (from_xlow == to_xlow && from_ylow == to_ylow) {
                    if (clb_grids[to_xlow][to_ylow].grid_type == CLB_TYPE) {
        ...
        case SINK:
            if (to_type == SOURCE) {   /* Feedthrough.  Not in code as yet. */
                if (from_xlow == to_xlow && from_ylow == to_ylow &&
                        clb_grids[to_xlow][to_ylow].grid_type == CLB_TYPE) {
                    ++num_adj;
                }
            }
            break;
        ...
        case IPIN:
            if (to_type == SINK && from_xlow == to_xlow && from_ylow == to_ylow) {
                if (clb_grids[from_xlow][from_ylow].grid_type == CLB_TYPE) {
...
static int pin_and_chan_adjacent(int pin_node, int chan_node)
{
    ...
    if (clb_grids[pin_x][pin_y].grid_type == CLB_TYPE) {
        if (chan_type == CHANX) {
